...
class bcf::p_v::reconfigure_neutron {
    ...
    file { '/etc/bigswitch/bcf_rest_client.py':
      ...
      require => EXEC['Openstack segment membership'],
    }
    exec { 'Openstack segment membership':
      command => "python /etc/bigswithch/bcf_rest_client.py -u ${bcf::params::openstack::bcf_username} -p ${bcf::params::openstack::bcf_password} -c ${rest_server} -m ${bcf::params::openstack::bcf_os_mgmt_tenant} -f ${bcf::params::openstack::deployment_id}",
      path    => '/usr/local/bin/:/usr/bin/:/bin',
